...
// Sources: user.delete.yaml, user.list.yaml, user.logout.yaml, user.unlock.yaml, user.configured.yaml, set.auto.admin.password.yaml, account.configuration.yaml
...
func NewUserConfiguredCommand(uuid string) *UserConfiguredCommand {
	return &UserConfiguredCommand{
		Command:     GenericCommandPayload{RequestType: UserConfiguredRequestType},
		CommandUUID: uuid,
	}
}
...
func init() {
	// associate our Request Type to a function for creating a command of that type
	newCommandFuncs[UserConfiguredRequestType] = func(uuid string) interface{} {
		return NewUserConfiguredCommand(uuid)
	}
}
...
func init() {
	// associate our Request Type to a function for creating a response of that type
	newResponseFuncs[UserConfiguredRequestType] = func() interface{} {
		return new(UserConfiguredResponse)
	}
}
